...
const OrdersTable: FC<Props> = ({ orders }) => {
    ...
    const COLUMNS = [
        ...
        t('orders.colTaxAmount'),
        t('orders.colTotalAmount'),
            ...
            <table className="w-full text-sm">
                ...
                {orders.map((order) => (
                    <tr key={order.id} className="group hover:bg-zinc-50/60 transition-colors duration-100">
                        ...
                        <td className="px-4 py-3.5 font-medium text-zinc-900 whitespace-nowrap">{formatAmount(order.subtotal)}</td>
                        <td className="px-4 py-3.5 text-zinc-600 whitespace-nowrap">{formatAmount(order.taxAmount)}</td>
                        <td className="px-4 py-3.5 text-zinc-600 whitespace-nowrap">{formatAmount(order.totalAmount)}</td>
                        <td className="px-4 py-3.5 font-mono text-xs text-zinc-400 whitespace-nowrap">
                            {/*{order.Latitude.toFixed(4)}, {order.Longitude.toFixed(4)}*/}
                        </td>
                        <td className="px-4 py-3.5 text-zinc-500 whitespace-nowrap">{formatDate(order.timestamp)}</td>
